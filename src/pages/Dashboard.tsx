...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    getUserCount().then(setActiveUsers).catch(() => setActiveUsers(null));
  }, []);
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Welcome back, <span className="text-blue-600 font-medium">{user?.name}</span>! Here's your analytics overview.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatsCard
            title="Total Uploads"
            value={totalUploads}
            change={totalUploads > 0 ? `${totalUploads} file${totalUploads !== 1 ? 's' : ''} uploaded` : 'No uploads yet'}
            changeType={totalUploads > 0 ? 'increase' : 'neutral'}
          ...
          <StatsCard
            ...
            value={totalCharts}
            change={totalCharts > 0 ? `Across ${totalUploads} file${totalUploads !== 1 ? 's' : ''}` : 'No charts yet'}
            changeType={totalCharts > 0 ? 'increase' : 'neutral'}
            icon={BarChart3}
          ...
          <StatsCard
            ...
            change={storageUsed > 0 ? 'Your uploaded files' : 'No storage used'}
            changeType="neutral"
            icon={HardDrive}
